...
$PYTHON_CMD $SCRIPT_PATH --model_name $model_name --steps 48 --nb_concepts $nb_concepts --verbose --test_name $test_name --decomposer $decomposer --decomposition_method $decomposition_method --split_point "$split_point" --metrics "$metrics" --unique_plot --freeshold $freeshold --nb_dataset_samples $nb_samples --reference_steps 48 --higher_decomposition --gradient_plot --kwargs '{"solver": "fro"}' --log_scale --dataset $dataset_name --dataset_split $dataset_split --text_field $text_field
